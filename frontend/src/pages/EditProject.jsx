...
const Section = ({ title, children }) => (
  <div className="card" style={{ padding: '1.75rem', marginBottom: '1.25rem' }}>
    <h2 style={{ fontSize: '1.05rem', marginBottom: '1.25rem', paddingBottom: '0.75rem', borderBottom: '1px solid var(--border)' }}>
      {title}
    </h2>
    {children}
  </div>
...
export default function EditProject() {
  ...
  const [form, setForm] = useState({
    title: '', description: '', requiredSkills: [], teamSize: 3,
    category: 'Web', status: 'open', githubRepo: '', tags: [],
  });
  ...
  const submit = async (e) => {
    ...
    setLoading(true);
    try {
      await api.put(`/projects/${id}`, { ...form, teamSize: Number(form.teamSize) });
  ...
  return (
    <div style={{ maxWidth: 680, margin: '0 auto' }} className="animate-fade">
      ...
      <h1 style={{ fontSize: '1.75rem', marginBottom: '1.5rem', fontWeight: 800 }}>Edit Project</h1>
      ...
      <form onSubmit={submit}>
        <Section title="Project Details">
          ...
          <Field label="Description *">
            <textarea name="description" className="input-field" value={form.description} onChange={handle} required rows={6} placeholder="Describe your project..." />
          </Field>
        ...
        <Section title="Tags (optional)">
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
            {form.tags.map((t) => (
              <span key={t} style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', background: 'var(--bg-muted)', padding: '0.25rem 0.75rem', borderRadius: '9999px', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: '4px' }}>
                #{t}
                <button type="button" onClick={() => removeTag(t)} style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', lineHeight: 1 }}>×</button>
              </span>
            ))}
            {form.tags.length === 0 && <span style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>No tags added yet</span>}
          </div>
        </Section>

        <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'flex-end' }}>
          <button type="button" className="btn-secondary" onClick={() => navigate(`/projects/${id}`)}>Cancel</button>
